...
class GitHubIngestion:
    ...
    def clone(self, github_url: str, progress_callback=None) -> RepoInfo:
        ...
        Returns RepoInfo with local path ready for parsing.
        """
        ...
        if os.path.exists(local_path):
            if progress_callback:
                progress_callback(f"📥 Repo already cached, pulling latest...")
            try:
                subprocess.run(
        ...
        if not os.path.exists(local_path):
            ...
            try:
                ...
                raise ValueError(
                    ...
                    f"Make sure the repo is public. Error: {e.stderr.decode()[:200]}"
                )

        if progress_callback:
            progress_callback(f"✅ Cloned to {local_path}")
    ...
    def get_repo_stats(self, local_path: str) -> dict:
        """Get basic stats about the cloned repo."""
    ...
    def cleanup(self, local_path: str):
        """Remove cloned repo to free space."""
        shutil.rmtree(local_path, ignore_errors=True)